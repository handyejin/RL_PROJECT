...
쉽게 실행하기 위한 얇은 진입점이다. 실제 알고리즘과 공통 경로/명령
생성 로직은 `src.agents.common.runner_config`에 둔다.

실행 예:
    PYTHONPATH=. python -m src.agents.run_interactive
    PYTHONPATH=. python -m src.agents.run_interactive --task vae --district ALL
    PYTHONPATH=. python -m src.agents.run_interactive --algorithm dqn --district ALL --candidate-top-k 3
"""
...
def run_training(args: argparse.Namespace, should_prompt: bool) -> None:
    ...
    print(f"실행 지역: {', '.join(districts)}", flush=True)
    print(f"데이터 분할: {args.split_mode}", flush=True)
